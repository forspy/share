...
*/
//2.派生类对象可以使用基类的公有方法（派生类不可以访问基类private成员，重要，只能通过基类方法）
//3.基类引用可以接住派生类的对象 如：ostream& a=cout,也可以是ostream& a=fout;（基类->派生类（如：ostream->ofstream））
/*
基类指针可以指向派生类对象
基类引用可以引用派生类对象
基类对象可以接住派生类对象（因为基类拷贝构造函数是一个引用），当然接住后的基类对象只能使用基类方法（无论有没有声明virtual）
以上为向上强制转换upcasting，表示范围的缩小，这是可行的；派生类是对基类的扩展；如果从BrassPlus派生出
